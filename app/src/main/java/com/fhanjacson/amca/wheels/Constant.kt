package com.fhanjacson.amca.wheels


class Constant {
    companion object {

        const val LOG_TAG = "PANTEQ"
        const val PICK_IMAGE_REQUEST_CODE: Int = 1337

        const val SETTING_LOGIN = "SETTING_LOGIN"

        const val SETTING_PROFILE = "SETTING_PROFILE"
        const val SETTING_CHANGE_PASSWORD = "SETTING_CHANGE_PASSWORD"
        const val SETTING_DELETE_PROFILE = "SETTING_DELETE_PROFILE"
        const val SETTING_LOGOUT = "SETTING_LOGOUT"



        const val VEHICLE_TYPE_SEDAN = "sedan"


        const val VEHICLE_TYPE_HATCHBACK = "hatchback"
        const val VEHICLE_TYPE_SUV = "suv"
        const val VEHICLE_TYPE_VAN = "van"
        const val VEHICLE_TYPE_PICKUP = "pickup"
        const val VEHICLE_TYPE_JEEP = "Jeep"
        const val VEHICLE_TRANSMISSION_AT = "at"

        const val VEHICLE_TRANSMISSION_MT = "mt"
        const val DATE_FORMAT_PATTERN_WITH_DAY = "E, d MMMM YYYY"

        const val DATE_FORMAT_PATTERN_WITH_TIME = "dd MMMM YYYY HH:mm:ss"

        const val ACTIVITY_TYPE_ACCOUNT_CREATED = "accountcreated"
        const val ACTIVITY_TYPE_CHANGE_PASSWORD = "changepassword"
        const val ACTIVITY_TYPE_ACCOUNT_DELETED = "accountdeleted"
        const val ACTIVITY_TYPE_UPDATE_PROFILE = "updateprofile"





    }
}